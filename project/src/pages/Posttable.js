...
const Posttable = () => {
    ...
    const DeleteItems=(id)=>{
        ...
        fetch(`http://localhost:7000/insertdelete/${id}`,{
            ...
            .then((res)=>res.json())
            .then((data)=>{
            toast.error("Deteleted successfully");
  ...
  <tbody >
            ...
            {ProductItems?.map((Item) =>(
                    ...
                    <td style={{textAlign:"center"}}>{Item.favoutfit}</td>
                    <td style={{textAlign:"center"}}>{Item.size}</td>
                    <td style={{textAlign:"center"}}>{Item.favclr}</td>
